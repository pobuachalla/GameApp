...
function buildPrintHTML() {
  ...
  h += buildPrintTransitionEfficiencyHTML();
...
// ─── TRANSITION EFFICIENCY ────────────────────────────────────────────────────
function buildPrintTransitionEfficiencyHTML() {
  ...
  let h = '<div class="pr-section"><div class="pr-section-title">Transition Efficiency</div><div class="pr-card">';
  h += '<div style="display:flex;gap:16px;justify-content:space-around;flex-wrap:wrap;">';
  if (positive.total > 0) {
    h += '<div style="flex:1;min-width:150px;">' + buildTransitionFunnelHTML('Positive Turnovers', [
    ...
    ], TEAM_US_COLOR, '#1F1F1F', '#6B6F66') + '</div>';
  }
  if (negative.total > 0) {
    h += '<div style="flex:1;min-width:150px;">' + buildTransitionFunnelHTML('Negative Turnovers', [
    ...
    ], TEAM_OPP_COLOR, '#1F1F1F', '#6B6F66') + '</div>';
  }
  h += '</div>';
  ...
  players.forEach(p => {
    h += html`<div class="pr-row"><span>${p.name}</span><span>`;
    if      (p.scoresCreated  > 0) h += `<span class="pr-tag" style="margin-right:4px;">${p.scoresCreated} score${p.scoresCreated!==1?'s':''} created</span>`;
    else if (p.shotsCreated   > 0) h += `<span class="pr-tag" style="margin-right:4px;">${p.shotsCreated} shot${p.shotsCreated!==1?'s':''} created</span>`;
    if      (p.scoresConceded > 0) h += `<span class="pr-tag" style="background:#F8D7D7;color:#991B1B;">${p.scoresConceded} conceded</span>`;
    else if (p.shotsConceded  > 0) h += `<span class="pr-tag" style="background:#F8D7D7;color:#991B1B;">${p.shotsConceded} shot${p.shotsConceded!==1?'s':''} against</span>`;
    h += '</span></div>';
  });
  ...
  if (insights.length) {
    h += '<div style="border-top:1px solid #E2E4DE;margin-top:10px;padding-top:8px;">';
    insights.forEach(line => { h += html`<div style="font-size:11px;color:#6B6F66;padding:2px 0;">${line}</div>`; });
    h += '</div>';
  }
...
function shareMatchReport() {
  ...
  imagesReady
    ...
    .catch(() => {
      area.classList.remove('capturing');
      toast("Couldn't generate the report PDF");
    });
}
